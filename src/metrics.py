...
def optimal_cutoff(estimator, X, y, ax, name):
    ...
    cutoff_df.plot.line(x="prob", y=["accuracy", "sensitivity", "specificity"], ax=ax)
...
def show_performance(estimator, X_train, y_train, X_test, y_test):
    ...
    print("\nTrain Recall Score: {}\nTest Recall Score: {}".format(train_recall_score, test_recall_score))
    ...
    axes = axes.ravel()
    # Show ROC
    disp = metrics.plot_roc_curve(estimator, X_train, y_train, ax=axes[0], name="Training")
    disp = metrics.plot_roc_curve(estimator, X_test, y_test, ax=axes[0], name="Testing")
    axes[0].set(title='Receiver operating characteristic')
    # Show Precision-Recall
    disp = metrics.plot_precision_recall_curve(estimator, X_train, y_train, ax=axes[1], name="Training")
    disp = metrics.plot_precision_recall_curve(estimator, X_test, y_test, ax=axes[1], name="Testing")
    axes[1].set(title='Precision-Recall')
    # Show Detection Error Tradeoff
    disp = metrics.plot_det_curve(estimator, X_train, y_train, ax=axes[2], name="Training")
    disp = metrics.plot_det_curve(estimator, X_test, y_test, ax=axes[2], name="Testing")
